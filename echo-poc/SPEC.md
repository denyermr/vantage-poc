...
- **Null model:** Seasonal climatological mean VWC, computed from training pool, applied by season. Phase 1 sealed-test RMSE = 0.178 cm³/cm³ (Method 1: training-pool seasonal means evaluated on n=36 sealed test). Phase 1c-Lean training-pool 5-fold CV RMSE = 0.1005 cm³/cm³ (Method 3: same seasonal-mean methodology evaluated within training pool via 5-fold hold-out, mean of per-fold RMSE; locked at SPEC v0.3.3 / DEV-1c-lean-007 / Block C-prime G3-Lean baselines lock per `phase1c-lean/g3_lean/baselines_locked.json`; not directly comparable to Phase 1 sealed-test number due to training-pool-vs-sealed-test variability gap).
