...
| [`DEV-1b-008`](DEV-1b-008.md) | 2026-04-19 | **Phase E Closure — G2 cross-configuration equivalence reframing.** After DEV-1b-004 (gradient arm dielectric) and DEV-1b-007 (published_table arm dielectric) resolved the dielectric category error, 9 of 12 active Sets A/B/C rows and 5/5 gradient rows still exceed the pre-registered ±0.5 dB / ±20 %·/·±0.1 dB tolerances. Phase E-1b diagnostic work (Υ²-attenuation probe on E.1/E.2, mechanism-decomposition probe on Sets A/B/C, Phase 1 honest-gates tier classification applied to G2) established that every significant residual traces to one of five pre-registered v0.1→v0.2 sub-module approximation choices, not implementation errors. Our Phase 1b MIMICS reproduces its own specification at machine precision (numpy↔PyTorch lockstep); it does not reproduce T94's MIMICS because the two use different sub-modules at five distinct layers (simplified-vs-full Dobson 1985, Oh 1992-vs-physical-optics, Rayleigh+sinc²-vs-UMF form factor, real-only-vs-dual-dispersion UEL, √σ°_oh-vs-Fresnel Γ). The original G2 pre-registration implicitly conflated implementation-correctness testing with cross-configuration equivalence testing; Phase E-1b made that implicit conflation explicit. | G2 (all three arms — closure classification). No impact on G1 / G3 / G4. | G2 classified as **Moderate Pass** under Phase 1 honest-gates tiered classification already committed across Vantage paper corpus. numpy_port FULL PASS (implementation correctness established); gradient MODERATE PASS (gradient-path liveness established via autograd ↔ FD internal consistency at 0.003 dB; residual vs T94 Table V(a) logged as Session F Dobson-fidelity diagnostic thread); published_table MODERATE PASS (characterised residuals). Per-row Δ dB + dominant mechanism + implicated v0.1 sub-module recorded in `g2_anchor_spec.md` v0.5 §"Per-row characterised residuals at Phase E closure". Frozen verdict JSON at `outputs/g2_equivalence_moderate_pass.json`. SPEC §14 sign-off block updated to reflect the three-arm Moderate Pass verdict. Phase 1b Block 2 (λ search + PINN-MIMICS trainer) cleared to begin on v0.1 physics unchanged. Session F physics promotions are **evidence-led from Phase 1b training diagnostics** per SPEC §11, not pre-committed at G2 closure. DEV-1b-006 (v0.2 physics promotion placeholder) retired at v0.5. Tolerance preserved (±0.5 dB remains the pre-registered criterion; failing rows recorded as characterised findings, not retrospectively tolerated) — same honest-gates discipline as Phase 1's Negative-with-diagnostic-characterisation outcome report. Sibling to DEV-1b-004, DEV-1b-005, DEV-1b-007. Cross-document consistency follow-up (White/Yellow/Green/Pitch Deck corpus audit) flagged to the user as a Session-E-close follow-up, not a blocker. |
| [`DEV-1b-009`](DEV-1b-009.md) | 2026-04-20 | **Phase 1b Session F-2 closure — λ-search dominance-constraint interpretation.** Implementation in `phase1b/lambda_search/run_f2.py` evaluated the SPEC §9 primary-dominance criterion under a strict all-reps-AND reading (inherited from Phase 1's `phase1/lambda_search.py` `n_violations==0` convention). Under the strict reading, 0 / 64 combinations satisfied the primary criterion → Tier 3 HALT. Under the verbatim pre-registered text of SPEC §9 line 317 ("averaged over all 10 reps at the 100 % training fraction"), the aggregation rule is the cross-rep mean of the per-rep final-window means, not per-rep AND. Under the verbatim-text reading, 16 / 64 combinations satisfy primary and the same 16 satisfy secondary → Tier 1 FULL_DOMINANCE. The dual-reading audit trail was foreseen and pre-mitigated: both readings are recorded per combination (`primary_dominance_all_reps` / `primary_dominance_mean_across_reps`, secondary equivalents). | F-2 tier classification. No gate-level impact (F-2 is a training step, not one of the four signed-off gates). Forward binding: cross-rep mean is the default aggregation rule for Phase 4 diagnostic thresholds D-1 through D-4 (SUCCESS_CRITERIA.md §6), subject to per-diagnostic revisit if an edge case surfaces during Phase 4 execution. | Adjudication: the verbatim pre-registered text binds. The mean-across-reps reading is the correct interpretation. Tier 1 FULL_DOMINANCE fires; selected combo_idx=1, λ = (λ_physics=0.01, λ_monotonic=0.01, λ_bounds=0.10), median_val_loss = 0.0353, mean_physics_fraction = 0.435. `run_f2.py` tier-classification block amended to drive on the mean-across-reps reading; dual-reading record-keeping preserved unchanged; result JSON regenerated from the on-disk per-rep histories under the binding reading (no re-training). Regression test at `tests/unit/test_f2_dominance_reading.py` pins the adjudication (5 tests). This is NOT a retroactive tolerance relaxation — the pre-registered numerical thresholds (L_data largest single term; L_physics > 10 % of total) and the three-tier fallback procedure are unchanged. Sibling to DEV-1b-008's implementation-vs-pre-registration-text finding at Phase E closure; same honest-gates discipline. |
...
[`DEV-1b-007.md`](DEV-1b-007.md), [`DEV-1b-008.md`](DEV-1b-008.md),
[`DEV-1b-009.md`](DEV-1b-009.md).
DEV-1b-006 ("v0.2 physics promotion") was a placeholder drafted in
Memo 2 of Session E Phase E scope-decision; retired at
`g2_anchor_spec.md` v0.5 / Phase E closure per DEV-1b-008. Phase E-1b
characterisation showed the intended scope was a five-way promotion
queue, not a single P2 item; speculative promotion before Phase 1b
training evidence would be post-hoc rationalisation of the tolerance
gap. Session F promotions are evidence-led from Phase 1b training
diagnostics per SPEC §11.
